...
named 'sales_recoded', includes the recoded columns and the 'transaction_id' column:
    ...
    %recode_cat_feat_subset(
		libname=mylib,
		dsname=sales,
		outdsname=sales_recoded,
		outlibname=work,
		start=1,
		end=10,
		keyvars=transaction_id
	);
...
%macro recode_cat_feat_subset(libname=, dsname=, outdsname=, outlibname=work, start=, end=, keyvars=);
    ...
    */
    ...
    %do i = &start %to &end;
        ...
        */
        proc sql;
            sysecho "1. creating lookup tables - %eval(&i. - &start. + 1)/%eval(&end. - &start. + 1) (%sysfunc(round(100*(%eval(&i. - &start. + 1)/%eval(&end. - &start. + 1)), 0.1))%)";
            create table &outlibname..lookup_&var as 
            select distinct trim(&var.) as n"&var.", monotonic() as int_value
    ...
    data &outlibname..&outdsname;
        ...
        %do i = &start %to &end;
            ...
            */
            ...
            sysecho "2. building mapping - %eval(&i. - &start. + 1)/%eval(&end. - &start. + 1) (%sysfunc(round(100*(%eval(&i. - &start. + 1)/%eval(&end. - &start. + 1)), 0.1))%)";
            rc = h.find();
